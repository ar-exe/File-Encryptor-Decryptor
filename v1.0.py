...
from test import find_char_position, generate_matrix
...
class FileEncryptorDecryptor:
    def __init__(self, root):
        ...
        self.operation_var = tk.StringVar(value="text")

        self.text_radio = tk.Radiobutton(self.operation_frame, text="Text Input", variable=self.operation_var,
                                         value="text", command=self.toggle_state)
        self.text_radio.grid(row=0, column=0)

        self.file_radio = tk.Radiobutton(self.operation_frame, text="File", variable=self.operation_var, value="file",
    ...
    def toggle_state(self):
        ...
        state = "disabled" if operation == "file" else "normal"
        for widget in self.text_frame.winfo_children():
            widget.configure(state=state)
        for widget in self.text_process_frame.winfo_children():
            widget.configure(state=state)

        state = "normal" if operation == "file" else "disabled"
